...
#
#3. I modify the hcl code of jle, so it can always get the correct 
#   address when it comes to the stage of decode.
...
int f_pc = [
	...
	M_icode == IMRMOVL && E_icode == IRMMOVL && D_icode == IJXX && D_ifun == C_LE && !M_Cnd : D_valP;
	M_icode == IJXX && !(W_icode == IRMMOVL && M_ifun == C_LE) && !M_Cnd : M_valA;
	# Completion of RET instruction.
	E_icode == IRET && M_icode == IPUSHL && W_icode == IMRMOVL : W_valM;
	W_icode == IRET && E_icode == INOP && D_icode == INOP : W_valM;
...
bool F_stall =
	...
	(IRET in { D_icode } && ! E_icode in { IPUSHL }) || 
		(IRET in { E_icode } && ! M_icode in { IPUSHL }) || 
			(IRET in { M_icode } && ! E_icode in { IPUSHL });
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd && !(W_icode == IMRMOVL && M_icode == IRMMOVL && E_ifun == C_LE)) ||
	# Stalling at fetch while ret passes through pipeline
