...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    switch(op){
        case A_ADD:
			if(val == 0) zero = TRUE;
			if(val < 0) sign = TRUE;
			if(((argA<0) == (argB<0)) && ((val<0) != (argA<0))) ovf = TRUE;
			break;
     	case A_SUB:
	        if(val == 0) zero = TRUE;
			if(val < 0) sign = TRUE;
			if(((argB<0) == (argA>0) && (val<0) != (argB<0))) ovf = TRUE;
			break;
	    case A_AND:
			if(val == 0) zero = TRUE;
			if(val < 0) sign = TRUE;
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    switch(cond){
		case C_YES:
			...
			break;
		case C_LE: 
		    if((sign ^ ovf) || zero) doit = TRUE;
			break;
		case C_L:
			if(sign ^ ovf) doit = TRUE;
			break;
		case C_E:
			if(zero) doit = TRUE;
			break;
...
stat_t nexti(y86sim_t *sim)
{
	...
	switch(icode){
		...
		case I_POPL:
			...
			regA = GET_REGA(reg);
			regB = GET_REGB(reg);
			//if(regA < 0 || regA > 0XF || regB < 0 || regB > 0xF) return STAT_INS;
			break;
	}
    ...
    switch (icode) {
      ...
      case I_RRMOVL:  /* 2:x regA:regB */
		...
		if(doit || ifun == 0) set_reg_val(sim->r, regB, valA);
		sim->pc = next_pc; 
		break;
      ...
      case I_RMMOVL: /* 4:0 regA:regB imm */
		...
		valE = valB + valC;
		if(!set_long_val(sim->m, valE, valA)) return STAT_ADR;
		sim->pc = next_pc;
		break;
      ...
      case I_RET: /* 9:0 */
		...
		valB = get_reg_val(sim->r, REG_ESP);
		valE = valB + 4;
		if(!get_long_val(sim->m, valA, &valM)) return STAT_ADR;
		set_reg_val(sim->r, REG_ESP, valE);
		sim->pc = valM;
		break;
