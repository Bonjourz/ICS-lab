...
int isLess(int x, int y) {
  int judgex = !(x >> 31);
  int judgey = !(y >> 31);
  int result1 = ((!judgex) & ((!judgex) & judgey));
  int result2 = !(judgex ^ judgey);
  int result3= !((~x + y) >> 31);
...
int isPower2(int x) {
  int y = !((x >> 31) | 0x0);
  int judge1 = !((((~x) + 1) & (x << 1)) ^ (x << 1));
  int judge2 = !!x;
  return judge1 & judge2 & y;
}
/* 
...
int reverseBytes(int x) {
  ...
  int mask3 = mask2 << 8;
  int mask4 = mask3 << 8;
  return ((x & mask1) << 24) | ((x & mask2) << 8) | ((x & mask3) >> 8) | (((x & mask4) >> 24) & mask1);
}
/* 
